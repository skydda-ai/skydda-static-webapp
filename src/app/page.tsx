...
const Home = () => {
  ...
  const handleEmailRequestModalClose = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setResMessage({
        ...
        success: res.message === SUCCESS_MESSAGE
      });
    ...
    <div className='relative min-h-screen p-0 md:p-5 bg-[#0D0D0D] overflow-hidden flex'>
        ...
        <main className='relative z-10 flex items-center flex-col gap-[60px] md:flex-row md:gap-[80px] mt-[60px]'>
          <div className='flex flex-col justify-center gap-[58px]'>
            ...
            <div className='flex flex-col md:flex-row gap-4'>
              <Button handleButtonClick={handleEarlyAccessButtonClick}>
      ...
      {isEmailRequestModalOpen && (
        <Modal
          ...
          onClose={handleEmailRequestModalClose}
        >
          <div>
            ...
            <form className='space-y-4' noValidate onSubmit={handleSubmit}>
              <input
                ...
                className={`w-full px-4 py-3 bg-black/40 border ${emailError ? 'border-red-400' : 'border-white/10'} rounded-lg text-white placeholder:text-white/40 focus:outline-none ${!emailError ? 'focus:ring-2 focus:ring-[#45266A]' : ''}`}
              />
              {emailError && (
